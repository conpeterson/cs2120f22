...
ANSWER HERE: 
If every person is mortal and Socrates is a person, then Socrates has to be mortal. 
This follows the Elimination AND rule.
-/
...
variables ed hannah mel : Triangle → Prop
variable likes_ed_hannah : ∀ (p : Person), Likes ed → mel
variable likes_hannah_mel : ∀ (p : Person), Likes hannah → mel
...
Answer: 
If (P and Q) is true that infers that both are true so (P or Q) must be true.
If (P and Q) is false, that can either be true or false depending on if at least one of them are false. 
True can never imply false in this case. This utilizes the Elimination ALL rule.
...
def answer : 
  Prop := ∀ (X Y: Person), Knows X → Y
    theorem knowsEveryone : answer := 
      begin
      unfold answer,
      assume Knows: true,
      assume t: true,
